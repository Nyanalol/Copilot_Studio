...
### Paso 2 · Definir las frases desencadenantes

En el nodo **Trigger**, añade frases como:
...
> 💡 Da variedad (formal/informal, con y sin tildes). El modelo generaliza, pero ejemplos diversos = mejor reconocimiento.
...
3. **Identify / Tipo de dato:** por ahora `User's entire response` (texto completo). En el Fascículo 04 lo cambiaremos por una **entidad personalizada** que valide el formato.
4. **Guardar en variable:** renómbrala a `NumeroPedido`.
...
1. Añade un nodo **Add a condition / Condición**.
2. Crea ramas según `NumeroPedido`:
   - **Rama 1 — Condición:** `NumeroPedido` *contiene* `100` → Mensaje:
   ...
   - **Rama 2 — Condición:** `NumeroPedido` *contiene* `200` → Mensaje:
     `🚚 Tu pedido ${NumeroPedido} ha sido ENVIADO y llegará en 2-3 días laborables.`
   - **Rama "Todas las demás / Else":** Mensaje:
     `No he podido localizar el pedido ${NumeroPedido}. ¿Puedes verificar el número? Si crees que es correcto, te paso con un agente.`

> ℹ️ Para insertar variables en un mensaje, usa el icono de variable (`{x}`) y elige `NumeroPedido`. La sintaxis mostrada `${...}` es ilustrativa.
